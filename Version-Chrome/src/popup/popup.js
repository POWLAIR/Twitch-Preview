...
elements.loginButton.addEventListener('click', () => {
    chrome.runtime.sendMessage({ type: 'INITIATE_AUTH' });
});
...
async function refreshStreams(forceRefresh = false) {
    ...
    try {
        ...
        const channelsResponse = await chrome.runtime.sendMessage({
        ...
        const streamsResponse = await chrome.runtime.sendMessage({
...
async function updateBadgeCount(count) {
    try {
        if (count > 0) {
            await chrome.browserAction.setBadgeText({ text: count.toString() });
            await chrome.browserAction.setBadgeBackgroundColor({ color: '#9146FF' }); // Couleur Twitch
        } else {
            await chrome.browserAction.setBadgeText({ text: '' }); // Enlève le badge si pas de streams
        }
    } catch (error) {
...
async function checkExistingAuth() {
    showLoading();
    try {
        const storage = await chrome.storage.local.get(['twitchToken']);
